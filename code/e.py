...
def evaluate_all_folds(ckpt_dir: str, gpu_id: int = 0):
    ...
    print("🚀 开始 32 折(样本)全量评测...")

    for fold in range(32):
        # 1. 智能匹配以 Fold 加编号开头的 .ckpt 文件
        ...
        print(f"\n[{fold+1}/32] 正在评估 Fold {fold} (权重: {os.path.basename(ckpt_path)})...")
        ...
        if label is not None and not isinstance(label, torch.Tensor):
            ...
            print(f"   ↳ {metrics_str} | Leiden ARI: (无标签)")

    # ==========================================
    # 汇总输出平均结果
    # ==========================================
    print("\n" + "="*50)
    print("📈 各项性能指标在整个数据集上的平均表现：")
    ...
    if len(results["ari"]) > 0:
        print(f"✅ 平均 Leiden ARI : {np.mean(results['ari']):.4f}  (共计 {len(results['ari'])} 个样本参与计算)")
    print("="*50 + "\n")

if __name__ == '__main__':
    # ----------------------------------------------------
    # 🔴 在此修改您的所有模型权重所在的主文件夹路径
    # 例如：模型权重存放为 model_checkpointsljx25/0.ckpt, model_checkpointsljx25/1.ckpt ...
    # ----------------------------------------------------
    CKPT_DIRECTORY = "/data/MorphoGeneST0/model_checkpointsljx30" 
    
    evaluate_all_folds(ckpt_dir=CKPT_DIRECTORY, gpu_id=0)