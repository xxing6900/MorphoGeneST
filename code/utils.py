...
def get_R(data1, data2, dim=1):
    adata1, adata2 = getattr(data1, 'X', data1), getattr(data2, 'X', data2)
    r1 = []
    for g in range(adata1.shape[dim]):
        r, _ = pearsonr(adata1[:, g], adata2[:, g])
        r1.append(r)
    return np.array(r1)

def cluster(adata_pred, label_gt):
    idx = (label_gt != 'undetermined')
    tmp = adata_pred[idx].copy()
    l = label_gt[idx]
    
    sc.pp.pca(tmp)
    sc.tl.tsne(tmp)
    
    kmeans = KMeans(n_clusters=len(set(l)), init="k-means++", random_state=0, n_init=10).fit(tmp.obsm['X_pca'])
    p = kmeans.labels_.astype(str)
    
    return p, round(ari_score(l, p), 4)
...
def calcADJ(coord, k=4, distanceType='euclidean', pruneTag='Grid'):
    ...
    for i in np.arange(nodes):
        tmp = coord[i, :].reshape(1, -1)
        distMat = distance.cdist(tmp, coord, distanceType)
        res = distMat.argsort()[:k + 1]
        for j in np.arange(1, k + 1):
            if pruneTag == 'Grid' and distMat[0, res[0][j]] <= 2.0:
                Adj[i][res[0][j]] = 1.0
            elif pruneTag == 'NA':
                Adj[i][res[0][j]] = 1.0
    return Adj
